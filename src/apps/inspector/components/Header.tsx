...
const Header: React.FC<HeaderProps> = ({ title, user, onNavigate }) => {
  ...
  const menuItems = [
    {
      label: 'Tạo Tally mới', target: 'CHON_LOAI_TALLY', icon: (
  ...
  return (
    <>
      <header className="bg-blue-600 text-white p-4 flex items-center justify-between w-full">
        ...
        {user && (
          <div className="flex items-center gap-2">
            <span className="hidden md:inline text-xs font-bold opacity-80 mr-2">{user.name}</span>
      ...
      {/* Sidebar Overlay - Sử dụng Fixed thay vì Absolute */}
      {isOpen && (
        <div
      ...
      {/* Sidebar Drawer - Sử dụng Fixed thay vì Absolute */}
      <div className={`fixed top-0 left-0 bottom-0 w-72 bg-white z-[110] shadow-2xl transform transition-transform duration-300 ease-out flex flex-col ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 bg-blue-600 text-white shadow-lg relative">
          ...
          <h2 className="text-xl font-black">{user ? user.name : 'Khách'}</h2>
        </div>
