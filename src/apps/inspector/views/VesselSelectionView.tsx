...
import { Vessel, Shift } from '../../../types/inspector';
...
const VesselSelectionView: React.FC<VesselSelectionViewProps> = ({ vessels, onSelect }) => {
  ...
  // const [vessels, setVessels] = useState<Vessel[]>([]); // Now using props
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col h-full">
          <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2 mb-4">
            ...
            Choose Vessel (Active: {vessels.length})
          </h3>
              ...
              >
                ...
                {vessels.map(v => (
                  <option key={v.id} value={v.id}>
                    {v.name} {v.voyage && v.voyage !== 'N/A' ? `- ${v.voyage}` : ''}
                  </option>
                ))}
              </select>
